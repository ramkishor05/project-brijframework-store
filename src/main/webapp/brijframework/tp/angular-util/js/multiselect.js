(function () {
    var uiMultiselect;
    try {
        uiMultiselect = angular.module("uiMultiselect")
    } catch (err) {
        uiMultiselect = null;
    }

    if (!uiMultiselect) {
        angular.module('uiMultiselect.services', ['ngResource', 'ngAnimate']);
        angular.module('uiMultiselect.controllers', []);
        angular.module('uiMultiselect.directives', []);
        angular.module('uiMultiselect.constants', []);
        angular.module('uiMultiselect',
            [
                'uiMultiselect.services',
                'uiMultiselect.controllers',
                'uiMultiselect.directives',
                'uiMultiselect.constants'
            ]);
    }
    ...
    var multiselect = function ($parse, $timeout, $filter, $document, $compile, $window, $position, optionParser,rootScope) {
        return {
            restrict: 'EA',
            ...
            fireRecalculating = function () {
                ...
                rootScope.$on('clearMultiSelectFilter',function() {
                    scope.clearFilter();
                })
    ...
    var multiselectPopup = function ($document) {
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                scope.$on('closeMultiFilter',function (){
                    element.removeClass('open');
                    scope.isOpen = false;
                    $document.unbind('click', clickHandler);
                })
                scope.toggleSelect = function () {
                    $(".dropdown-datepicker").css("display","none");
                    ...
                    if (dropdownHeight <= 0 && dropdownWidth <= 0) {
                        ...
                        // Memory clean up - also, if you don't remove the clone from the DOM, IE11 increases the height of the HTML DOM element
												// (buggy piece of junk!)
    ...
    angular.module("uiMultiselect").run(["$templateCache", function ($templateCache) {
        $templateCache.put("template/multiselect/multiselectPopup.html",
            "<div class=\"btn-group col-md-12 no-padding\" ng-class=\"{ dropup: dropup, single: !multiple }\">" +
                "<button type=\"button\" class=\"btn btn-block multi-select-filter btn-light text-right  dropdown-toggle col-md-12\" ng-click=\"toggleSelect()\" ng-disabled=\"disabled\" ng-class=\"{'has-error': !valid()}\">" +
                    "<span class=\"text-left col-md-11 no-padding text-length\" ng-bind=\"header\"></span>" +
                    "<i class=\"fa fa-filter fa-fw padding-left-10\"></i>" +
                "</button>" +
                "<ul class=\"dropdown-menu multi-select-popup col-md-12\" ng-show=\"isOpen && !moveInProgress\" ng-style=\"{ true: {top: position.top +'px', left: position.left +'px'}, false: {}}[appendToBody]\" style=\"display: block;max-height: 220px;overflow: auto;\" role=\"listbox\" aria-hidden=\"{{!isOpen}}\">" +
                    "<li ng-if=\"enableFilter\" class=\"filter-container\">" +
                        "<div class=\"form-group has-feedback filter\">" +
                            ...
                            "<span class=\"glyphicon glyphicon-remove-circle cursor-pointer form-control-feedback\" ng-click=\"clearFilter()\"></span>" +
                        "</div>" +
                    "</li>" +
                    "<li ng-show=\"multiple\">" +
                        "<button type=\"button\" class=\"btn-link btn-small\" ng-click=\"checkAll()\"><i class=\"icon-ok\"></i> Select all</button>" +
                        "<button type=\"button\" class=\"btn-link btn-small pull-right\" ng-click=\"uncheckAll()\"><i class=\"icon-remove\"></i> Clear</button>" +
                    "</li>" +
    ...
    multiselect.$inject = ['$parse', '$timeout', '$filter', '$document', '$compile', '$window', '$uibPosition', 'multiselectParser','$rootScope'];
    multiselectPopup.$inject = ['$document'];

    angular
        .module("uiMultiselect.services")
        .factory('multiselectParser', multiselectParser);

    angular
        .module('uiMultiselect.directives')
