      ...
      var dataPromise = $q.when(data).then(function (responseData) {
        ...
        angular.forEach(arrData, function (oldRow, index) {
          ...
          angular.forEach(row, function (field, key) {
        	if(options.keyArray.indexOf(key) !== -1){
      ...
      controller: [
        ...
        function ($scope, $element, $attrs, $transclude) {
          ...
          function getBuildCsvOptions() {
            ...
            if(angular.isDefined($attrs.keyArray)) { options["keyArray"] = angular.isArray($scope.keyArray) ? $scope.keyArray() : $scope.keyArray(); }
            else { options["keyArray"] = angular.isArray($scope.data) ? Object.keys($scope.data[0]) : Object.keys($scope.data()[0]);};
