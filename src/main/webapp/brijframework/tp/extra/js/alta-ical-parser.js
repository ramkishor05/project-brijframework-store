 ...
 * Proof of concept method of reading icalendar (.ics,.ical, .icalendar) files with javascript.
 *
 * @author: Maneesh Tripathi
 */
function ICalParser(icalStr){
    ...
    this.makeParamObjsFromDicts = function(paramArray){
        var paramObj = {};
     ...
     */
    this.parseICAL = function(data,callback){
        //Ensure cal is empty
        this.events = [];
        ...
        for(var i=0;i<cal_array.length;i++){
            ...
            if(in_event && ln == 'BEGIN:VALARM'){
            	in_alram = true;
            }
            
            //If we are in an event
            if(in_event && !in_alram){
                ...
                if(type == 'RRULE'){
                    val = this.makeParamObjsFromDicts(val.split(";"));
                }
                //Convert timestamp
     ...
     * @param ical  String
     *!/
    this.load = function(ical_str){
        var tmp_this = this;
        this.raw_data = null;
        //store the data and invoke the parser
        tmp_this.raw_data = ical_str;
        tmp_this.parseICAL(ical_str);
    }*/
